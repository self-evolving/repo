...
4. After you create a repository from the template, Sepo may automatically start setup Actions and open or update a `Sepo setup check` issue with check results and follow-up setup instructions. If that does not happen, run `Agent / Onboarding / Check Setup` from GitHub Actions; it creates the built-in `agent/*` trigger labels if they are missing and opens or updates the setup check issue.
