...
When an explicit `/implement` request on a pull request or discussion creates a tracking issue, the router fetches target context first and passes it into a metadata-only agent prompt to synthesize the issue title and body. The slash command approves the route; it is not copied into the title. If metadata generation is unavailable or invalid, the issue falls back to `Implement requested change`.
