...
`agent-self-approve`, including the eligible case where the synthesis recommends
`HUMAN_DECISION` but its final verdict is `SHIP`. Manual heuristic PR starts can
also consult `agent-self-approve` when self-approval is enabled. A self-approval
