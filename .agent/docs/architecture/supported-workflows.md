...
All agent runs can also receive the optional `AGENT_CROSS_REPO_PAT` secret as
`INPUT_SECONDARY_GITHUB_TOKEN`. This secondary credential is for explicit
cross-repository operations and does not replace the primary same-repository
token, including the dedicated `/install` primary token described above.
