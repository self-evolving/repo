...
routes continue using the standard GitHub auth resolver. The install fork/PR
helper owns target fork, worktree, push, and PR mechanics; the prompt owns
target-specific inspection, copy, validation, commit, and PR body judgment.
Source-repo memory is disabled for install runs so that install token cannot
write `agent/memory`.
