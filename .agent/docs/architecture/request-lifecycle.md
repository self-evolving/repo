...
- Explicit `/add-rubrics` requests use the same implementation workflow but set `base_branch=agent/rubrics` and the add-rubrics prompt. The workflow keeps Sepo runtime files on the default branch and runs the agent in a separate rubrics worktree, then opens a draft proposal PR targeting `agent/rubrics`.
...
Direct implementation and PR-fix runs publish a live progress comment on issue and pull request surfaces by default, while answer runs use report-only progress by default. The shared `run-agent-task` action starts a best-effort reporter before the agent run, passes it the agent's ACP stream file, and tears it down after the run. Progress comments are temporary for all three routes. Finalization posts a new durable result with the resolved GitHub identity (normally the hosted or self-managed Sepo App), then best-effort deletes the verified temporary progress comment with the job token. It posts the final result first so cleanup can never erase the only response; if the resolved-auth post fails, it preserves availability by merging the result into the temporary comment without retaining progress activity. If progress is disabled or no progress comment was created, the route uses the same direct final-response path and has nothing to delete. An authorized 👎 reaction from the requester, repository owner, member, or collaborator first marks cancellable progress comments as cancelled, then requests GitHub Actions cancellation. `AGENT_PROGRESS_POLICY` controls whether each route has cancellable (`enabled`), report-only, or disabled progress; review runs remain disabled by default. Runs with explicit orchestration context ignore normal route overrides, default to no progress comment, and report state through orchestrator handoff or status comments unless `orchestration_mode` explicitly opts into report-only progress.

Progress comment creation, repeated updates, cancellation rendering, and temporary-comment deletion authenticate with the job-scoped `GITHUB_TOKEN`. GitHub [does not create new workflow runs for events emitted by that token](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/trigger-a-workflow#triggering-a-workflow-from-a-workflow), so human `created` and `edited` comment triggers can remain enabled without recursive progress runs. Implement, PR-fix, and answer agent processes receive that same repository-scoped primary token rather than the resolved App or `AGENT_PAT` credential; the optional read-only secondary-token contract is unchanged. Resolved App or PAT credentials remain available to deterministic workflow steps and normally author durable final results. Each such result carries a deterministic hidden final-response marker. The entrypoint rejects that marker before runner allocation and `extract-context` rejects it again, so PAT-authored results that look like user comments cannot become implicit follow-ups or authorize command suggestions. GitHub may still create one skipped workflow-run record for the final comment, but it cannot produce a progress-trigger loop. Fix-PR finalization continues to clean up older status comments across both resolved and job-token identities without matching human or unrelated-bot comments.
