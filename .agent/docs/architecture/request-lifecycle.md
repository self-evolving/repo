...
Direct implementation and PR-fix runs publish a live progress comment on issue and pull request surfaces by default, while answer runs use report-only progress by default. The shared `run-agent-task` action starts a best-effort reporter before the agent run, passes it the agent's ACP stream file, and tears it down after the run. If the reporter created a comment, the final issue/PR response step patches that same comment with the substantive result. Final `/answer` comments omit the progress activity, while implementation and PR-fix status comments keep it in a collapsed activity log; if no progress comment exists, the workflow falls back to posting the normal final comment. An authorized 👎 reaction from the requester, repository owner, member, or collaborator first marks cancellable progress comments as cancelled, then requests GitHub Actions cancellation. `AGENT_PROGRESS_POLICY` can disable the comment or switch a route to `report-only`; review runs remain disabled by default. Runs with explicit orchestration context ignore normal route progress overrides, default to no progress comment, and report state through orchestrator handoff or status comments unless `orchestration_mode` explicitly opts into report-only progress.

Progress comment creation, repeated updates, cancellation rendering, and final-response replacement authenticate with the job-scoped `GITHUB_TOKEN`. GitHub [does not create new workflow runs for events emitted by that token](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/trigger-a-workflow#triggering-a-workflow-from-a-workflow), so human `created` and `edited` comment triggers can remain enabled without recursive progress runs. Implement, PR-fix, and answer agent processes receive that same repository-scoped primary token rather than the resolved App or `AGENT_PAT` credential; the optional read-only secondary-token contract is unchanged. The entrypoint's bot condition is still a useful runner-allocation guard, but it runs only after GitHub has created the workflow-run object and therefore cannot prevent Actions-history churn on its own. Resolved App or PAT credentials remain available to the separate deterministic workflow steps that need the Sepo identity or broader lifecycle permissions, and a failed progress merge still falls back to the normal final-comment path. Fix-PR finalization cleans up prior status comments once with each distinct resolved and job-token identity, preserving cleanup across the author transition without matching human or unrelated-bot comments.
