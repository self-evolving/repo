...
- Rubric requests dispatch `Agent / Rubrics / Add`, which validates add-or-update rubric edits, opens a proposal PR targeting `agent/rubrics` by default, commits directly only when the request explicitly asks for that, and posts a concise result summary.
