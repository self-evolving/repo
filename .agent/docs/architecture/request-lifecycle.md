...
- Explicit implementation requests (`@sepo-agent /implement ...` or the `agent/implement` label) skip the approval comment. The router creates a tracking issue if the surface isn't already an issue and dispatches `agent-implement.yml` directly, since the explicit trigger is itself the approval. Tracking metadata is derived locally without another agent session. For slash commands, the title comes from the active request after removing the mention and route command; it is limited to 70 characters and falls back to `Implement requested change` when no usable command text remains. Label triggers use the source issue or PR title and treat its body only as context, so stale slash-command prose cannot imply stacking. The body retains the original request or source context and links back to its source. For PR slash commands that explicitly ask for stacked or follow-up PR work, the router passes the source PR as `base_pr`; independent requests use the default branch. If the inferred source PR is closed, merged, or from a fork, the router omits `base_pr`, adds a base-branch note to the tracking issue, and keeps that PR only as context. Access control (`AGENT_ACCESS_POLICY`) still applies to the `implement` route. The explicit path also passes a session-fork hint from the original target's `answer/default` thread, so implementation can continue from a prior answer session when that bundle exists.
