...
| `agent-add-rubrics.yml` (`Agent / Rubrics / Add`) | explicit `/add-rubrics`, triaged rubric requests, `agent/add-rubrics`, `workflow_dispatch` | Proposes add-or-update rubric YAML edits in a draft PR targeting `agent/rubrics` | No, opens PR |
...
`Agent / Rubrics / Add` is the user-facing proposal path for conversation requests. It reads existing rubrics from a separate `agent/rubrics` checkout, prefers updating active rubrics over creating near-duplicates, validates the full rubric checkout, pushes a stable proposal branch, and opens or reuses a draft PR targeting `agent/rubrics`. The main workflow checkout remains on trusted default-branch runtime code; `agent/rubrics` is only the edited target checkout. During proposal generation, the model process does not receive the write token, and mounted memory/rubric checkouts have unauthenticated remotes. Deterministic post-processing is responsible for pushing the proposal branch after validation.
...
Dedicated rubric-initialization and rubric-update runs pass `rubrics_mode_override: enabled`, so they can write the branch even when the repository default is read-only. Add-rubrics uses `rubrics_mode_override: read-only` and opens a PR from a proposal branch instead of committing directly. Only rubric initialization bootstraps a missing branch; add-rubrics and rubric update expect `agent/rubrics` to already exist.
