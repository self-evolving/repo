...
| `AGENT_SELF_IMPROVEMENT_ENABLED` | Set to `true` to enable `agent-self-improvement.yml`. The workflow wakes on its cron or manual dispatch, honors `AGENT_SCHEDULE_POLICY` for explicit scheduled-run disables, asks the planner to choose `new_issue`, `continue_issue`, or `continue_pr`, then deterministically creates/comments and dispatches `agent-orchestrator.yml`. It intentionally does not use memory-cursor throttling or open-proposal backlog gating. Retry/rerun writes are deduped only by the current run marker and authenticated Sepo authorship; proposal issue reuse and continuation trace suppression both require the authenticated Sepo actor as author. Manual dispatches use the real dispatcher's repository association; scheduled runs are system-authorized by the repository owner opt-in. |
