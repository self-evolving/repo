...
| `AGENT_TEST_RUNS_ON` | Optional JSON array string for `test-scripts.yml` runner selection. It falls back to `AGENT_RUNS_ON`, then `["ubuntu-latest"]`. Because `test-scripts.yml` runs on `pull_request`, self-hosted runner labels here allow PR code to run on that runner. |
...
| `AGENT_SELF_IMPROVEMENT_ENABLED` | Set to `true` to enable `agent-self-improvement.yml`. The workflow wakes every 6 hours on its cron or by manual dispatch, honors `AGENT_SCHEDULE_POLICY` for explicit scheduled-run disables, asks the planner to choose `new_issue`, `continue_issue`, or `continue_pr`, then deterministically creates/comments and dispatches `agent-orchestrator.yml`. It intentionally does not use memory-cursor throttling or open-proposal backlog gating. Retry/rerun writes are deduped only by the current run marker and authenticated Sepo authorship; proposal issue reuse and continuation trace suppression both require the authenticated Sepo actor as author. Continuation targets must be authored by the authenticated Sepo actor or another trusted repository actor. Manual dispatches use the real dispatcher's repository association; scheduled runs are system-authorized by the repository owner opt-in. |
