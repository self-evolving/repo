...
| `AGENT_FAILURE_REPORT_ENABLED` | Controls automatic failed-agent-run reports: `auto`, `true`, or `false`. `auto` reports public repository failures and skips private repositories. |
| `AGENT_FAILURE_REPORT_REPOSITORY` | Repository that receives failed-agent-run Discussion reports. Defaults to `self-evolving/repo`. |
| `AGENT_FAILURE_REPORT_DISCUSSION_CATEGORY` | Discussion category for failed-agent-run reports. Defaults to `Bug Report`. |
...
| `AGENT_FAILURE_REPORT_TOKEN` | Optional token scoped to `AGENT_FAILURE_REPORT_REPOSITORY` when using a custom failure-report intake repository |
