...
For Codex GPT-5 models, Sepo accepts provider-neutral `model` plus `reasoning_effort` policy entries and passes the effective ACP model id to acpx. For example, `{ "model": "gpt-5.6-sol", "reasoning_effort": "max" }` is sent to Codex ACP as `gpt-5.6-sol/max`, matching the model ids advertised by the bundled `@agentclientprotocol/codex-acp` adapter. If a Codex model already includes an effort suffix such as `gpt-5.6-sol/max`, Sepo treats that model id as authoritative and does not send a separate `thought_level` setting. Claude and unknown/custom Codex model ids keep using the separate reasoning-effort path.

A model-only policy override that changes away from the bundled Codex default does not inherit the bundled `max` effort; set `reasoning_effort` explicitly when a custom model should receive one, or set it to `null` / `""` to clear an inherited policy effort.
