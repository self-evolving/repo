...
| `AGENT_DISPLAY_MODEL` | Optional `true` / `false` toggle for appending compact run metadata to direct agent response comments that use the standard response posting helpers. When enabled, Sepo normalizes provider aliases and encoded Codex reasoning from session and requested-model evidence, then renders <code>provider &#124; model &#124; reasoning effort &#124; runner</code>; empty optional values are omitted, and an empty model is shown as `default model`. Defaults to `true`; set to `false` to hide the footer. |
