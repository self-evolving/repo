...
| `AGENT_AUTOMATION_MODE` | Orchestrator decision mode. Defaults to `agent` for planner-backed orchestration validated by runtime policy. Deterministic routing is internal policy and worker-chain behavior. Compatibility alias: `heuristics` and legacy `true` run the deterministic worker chain; explicit `false` or legacy `disabled` values fall back to that worker chain only for explicit `/orchestrate` chains. See [Agent orchestrator](../architecture/agent-orchestrator.md). |
