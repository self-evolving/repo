...
Sepo does not maintain a general model catalog. The resolver checks the Sepo-owned GitHub registry by default so installed repos can receive refreshed defaults between infrastructure updates; fetch failures, timeouts, or invalid remote payloads fall back to the bundled file. Use `AGENT_MODEL_POLICY` when a repository needs different models, route-specific choices, or reasoning effort overrides.
