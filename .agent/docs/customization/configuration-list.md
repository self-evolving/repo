...
| `AGENT_CODEX_AUTH_MODE` | Set to `managed` to restore encrypted ChatGPT-managed Codex authentication from `CODEX_AUTH_STATE` on ephemeral runners and persist refreshed state after each Codex task. Requires `CODEX_AUTH_KEY` and a GitHub token with repository Variables read/write permission. Keep unset for API-key or persistent local authentication. |
| `CODEX_AUTH_STATE` | Repository variable containing only AES-256-GCM ciphertext for managed Codex account authentication. Create it with `npm --prefix .agent run seed:codex-auth -- --repo owner/repo --auth-file /path/to/auth.json`; never put plaintext `auth.json` in a variable. |
...
| `CODEX_AUTH_KEY` | Base64-encoded 32-byte Actions secret used only to decrypt/encrypt `CODEX_AUTH_STATE` for the advanced managed Codex account-auth mode. Treat it as a credential and never store it as a variable or commit it. |
