...
| `AGENT_ALLOW_SELF_MERGE` | Separate opt-in gate for `agent-self-merge.yml`. Defaults to `false`; when enabled, orchestrated self-approval can proceed to deterministic merge or GitHub auto-merge only after current-head self-approval and merge preflight checks pass. |
