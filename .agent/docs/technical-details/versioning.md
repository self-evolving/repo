...
`.agent/CHANGELOG.md` is the canonical Sepo changelog. It is written at release time: the release flow drafts each version's section from the pull requests merged since the previous release tag. Pull requests do not add or maintain an `Unreleased` section; the pull request description is the per-change record.
