...
boundary: direct user-launched orchestrator starts derive requester identity
from `github.actor` even when a `requested_by` input is supplied. Child and
parent resume dispatches preserve `requested_by` for traceability and for
route-specific guards such as self-approval's requester-author check, but they
do not need to thread requester association and route policy through every
downstream workflow.
