...
Active automation chains also use the `agent-auto-running` label as a visible opt-out switch. The first dispatched handoff creates and adds the label on the destination issue or pull request. Later handoffs require the label to still be present; if a maintainer removes it while a chain is running, the next orchestrator pass stops instead of dispatching another action. When the orchestrator decides to stop normally, it removes the label best-effort. Action workflows report both successful and terminal outcomes to `agent-orchestrator.yml` through their final handoff dispatch step, so label cleanup follows the same orchestrator stop path for failures and unsupported targets. Removing the label does not cancel an action that is already running.
