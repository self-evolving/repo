...
stateDiagram-v2
    ...
    [*] --> SelfApprove: /orchestrate on PR + AGENT_ALLOW_SELF_APPROVE=true
    [*] --> Review: /orchestrate on PR + self-approval disabled
    [*] --> FixPR: /orchestrate on PR with CHANGES_REQUESTED + self-approval disabled
...
- pull request target with `AGENT_ALLOW_SELF_APPROVE=true`: dispatch
  `agent-self-approve` after confirming the PR is open; the self-approval
  workflow owns the trusted current-head `SHIP` check and can block when the
  final decision should stay with a human
- pull request target with `CHANGES_REQUESTED` and self-approval disabled:
  dispatch `fix-pr`
- other open pull request targets with self-approval disabled: dispatch
  `review`
...
Review-originated `fix-pr` handoffs carry explicit task context when available. The review dispatcher derives it from the latest review synthesis action items, and heuristic mode falls back to a conservative instruction to address only unresolved review synthesis action items while ignoring optional INFO notes and metadata-only polish. When the latest review synthesis recommends `HUMAN_DECISION`, non-`SHIP` final verdicts stop for a human. If the final verdict is `SHIP` and self-approval is enabled, the dispatcher keeps the `SHIP` conclusion, adds explicit self-approval handoff context, and lets `agent-self-approve` approve, request changes with concrete context, or block for a human. Manual PR `/orchestrate` starts with a `CHANGES_REQUESTED` review decision use separate context that tells `fix-pr` to address the latest unresolved requested-change review comments instead of the review-synthesis fallback only when self-approval is disabled. When self-approval is enabled, manual heuristic PR starts can consult `agent-self-approve` and let that workflow approve, request changes, or block. Self-approval `REQUEST_CHANGES` handoffs preserve the approval agent's handoff context as the `fix-pr` task. Self-approval `APPROVED` handoffs dispatch `agent-self-merge` only when `AGENT_ALLOW_SELF_MERGE=true`.
