...
- `OPENAI_API_KEY`, `CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`, and/or Secret-only `PI_AUTH_JSON_B64` / `PI_AUTH_JSON` as provider credentials
...
2. add at least one provider credential secret: `OPENAI_API_KEY`,
   `CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`, and/or Secret-only
   `PI_AUTH_JSON_B64` / `PI_AUTH_JSON`
