...
On self-hosted runners, an explicit `AGENT_DEFAULT_PROVIDER=codex`, `AGENT_DEFAULT_PROVIDER=claude`, or matching `AGENT_MODEL_POLICY` provider choice is treated as an operator choice. The provider resolver will select that provider even if the matching repository secret is absent, so single-agent runs and review synthesis can use local Codex or Claude authentication already configured on the machine. In `auto` mode, provider detection still relies on repository secrets and prefers Codex when both provider secrets are present. The same model policy can pass a provider-specific `model` to acpx. The review workflow still attempts explicit Claude and Codex reviewer lanes; provider and model resolution controls only the synthesis step that combines successful reviewer outputs.
