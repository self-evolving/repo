...
At a high level, the runner host needs Node support compatible with `.github/actions/setup-agent-runtime`, `git`, `gh`, `jq`, `curl`, `bash`, and network access. It also needs either repository secrets for the selected agent providers or local provider authentication available to the same user that runs the GitHub runner. Docker is optional unless your workflows require it. When Pi is selected, setup can install the `pi` CLI from `@mariozechner/pi-coding-agent` if it is not already present.

## Provider auth note

On self-hosted runners, an explicit `AGENT_DEFAULT_PROVIDER=codex`, `AGENT_DEFAULT_PROVIDER=claude`, `AGENT_DEFAULT_PROVIDER=pi`, or route-specific `AGENT_MODEL_POLICY` provider override is treated as an operator choice. The provider resolver will select that provider even if the matching repository secret is absent, so single-agent runs and review synthesis can use local Codex, Claude, or Pi authentication already configured on the machine. In `auto` mode, provider detection still relies on repository secrets and chooses Codex when `OPENAI_API_KEY` is configured, otherwise Claude when either `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` is configured, otherwise Pi when `PI_AUTH_JSON_B64` or `PI_AUTH_JSON` is configured. The same model policy can pass a provider-specific `model` to acpx. The review workflow still attempts explicit Claude and Codex reviewer lanes; provider and model resolution controls only the synthesis step that combines successful reviewer outputs.

For Pi OAuth/subscription auth, a persistent self-hosted `~/.pi/agent/auth.json` is preferred over `PI_AUTH_JSON` / `PI_AUTH_JSON_B64`. The secret-based path restores auth into `$RUNNER_TEMP/pi-agent` for a single run, which is convenient for testing and API-key-style Pi auth entries, but any OAuth refresh-token rotation updates only the temporary file and can leave the repository Secret stale for later runs.
