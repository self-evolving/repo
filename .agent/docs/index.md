...
For direct `/answer`, `/implement`, and `/fix-pr` runs on issues or pull requests, live progress is temporary and authored with the non-recursive Actions token. When the run finishes, Sepo posts the durable result with its resolved GitHub identity—normally the Sepo App—and then deletes the temporary progress comment. If final posting fails, the progress comment becomes the fallback result instead; when progress is disabled, Sepo posts the final result directly. Orchestrated chains use handoff and status comments by default; set `AGENT_PROGRESS_POLICY` with `orchestration_mode: "report-only"` to opt into non-cancellable progress comments for orchestrated runs. The original requester, repository owner, member, or collaborator can react 👎 on cancellable progress comments to stop the in-flight run.
