...
| Install | `install` | `.github/prompts/agent-install.md` | first-class `/install` route for authorization; runs the dedicated install prompt with the install-only token; issue-template requests close after a target install PR is published |
