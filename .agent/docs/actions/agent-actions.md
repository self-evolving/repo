...
| Config | `config` | `.github/prompts/agent-config.md` | `Agent / Repo Config` workflow or explicit `/config` mention interprets natural language into a validated repository Actions variable plan |
...
The config route changes only documented Sepo repository Actions variables. It
uses an agent to translate natural language into a small JSON plan, then a
deterministic CLI validates the allowlist and applies `set`/`unset` operations
through GitHub's Actions variables API. `Agent / Repo Config` defaults to dry
run from the Actions page; explicit `/config` and `/configure` mentions apply
the validated plan for authorized users. By default this route excludes
`CONTRIBUTOR` access unless `AGENT_ACCESS_POLICY.route_overrides.config`
