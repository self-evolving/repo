...
For issue and pull request bodies, issue and pull request comments, and pull request review comments, the portal attempts the initial 👀 reaction immediately after checkout. This best-effort path uses `github.token`, raw event IDs, and REST; it ignores mentions in fenced code, inline code, and quoted lines. A successful early reaction suppresses the later GraphQL reaction so the Actions and resolved App/PAT identities do not both react. If the REST request fails, the mention is not live, or the event is a discussion, discussion comment, or pull request review body, the normal post-setup GraphQL path remains available.
