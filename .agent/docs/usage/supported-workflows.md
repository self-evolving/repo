...
Concurrency groups guard against pending-run eviction: GitHub keeps at most one pending run per group and cancels the previously pending run whenever a newer one queues. Bot-sender events (including Sepo's own progress-comment edits) and non-`agent/*` label events therefore run in unique throwaway groups where they cannot evict a queued human-triggered run. Human events key on the triggering comment or review when one exists, so mentions in different comments on the same target run concurrently; comment-less surfaces keep per-target serialization, where a newer edit of the same surface replacing a pending run is the intended dedupe. Write routes keep their own non-lossy locks (`queue: max` retains up to 100 queued requests in FIFO order instead of replacing the single pending run): `agent-implement.yml` serializes per issue at the workflow level, `agent-fix-pr.yml` carries a job-level per-PR group because workflow-level concurrency is not applied on the `workflow_call` path, and the router's `install` and `skill` jobs serialize per source target since both run with write-capable credentials. Parallel answer runs on one target may race on session-bundle registration; session policy is resume-best-effort, so the cost is reduced continuity, not corruption.
