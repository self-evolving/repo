...
| `agent-cache-seed.yml` | `push` to the default branch touching `.agent/**` or the setup action, `workflow_dispatch` | Warms the exact-key runtime setup caches from the trusted default branch so user-facing runs rarely pay the first-miss cost | None |
...
For issue and pull request bodies, issue and pull request comments, and pull request review comments, the portal attempts the initial 👀 reaction as its first step, before checkout and runtime setup, using only `jq`, `curl` (bounded to a 10-second request within a one-minute step), raw event IDs, and REST with `github.token`. The inline check mirrors the boundary-aware mention validation (fenced code — including list-nested fences, handled more conservatively than the runtime parser — inline code, and quoted lines are ignored; handle boundaries are enforced, so text that merely contains the handle as a prefix — such as the app account name `@sepo-agent-app` — does not trigger a reaction) and is best-effort; a successful early reaction suppresses the later resolved-identity reaction so two accounts do not both react. Edited events, discussions, and pull request review bodies keep the normal post-setup reaction path.
...
Concurrency groups guard against pending-run eviction: GitHub keeps at most one pending run per group and cancels the previously pending run whenever a newer one queues. Bot-sender events (including Sepo's own progress-comment edits) and non-`agent/*` label events therefore run in unique throwaway groups where they cannot evict a queued human-triggered run. Human events key on the triggering comment or review when one exists, so mentions in different comments on the same target run concurrently; comment-less surfaces keep per-target serialization, where a newer edit of the same surface replacing a pending run is the intended dedupe. Accepted requests queue non-lossily (`queue: max` retains up to 100 queued requests in FIFO order instead of replacing the single pending run): `agent-implement.yml` serializes per issue, `agent-fix-pr.yml` carries both its workflow-level lock and a belt-and-braces job-level per-PR group for the `workflow_call` path, the router's `skill` job serializes per source target, `install` serializes repository-wide because different source issues can converge on the same external install target, the orchestrator queues rather than evicts overlapping accepted requests, and human `agent/*` label runs use their own non-lossy per-target `agent-label-*` groups so comment-less mention events (which keep single-pending replacement as edit dedupe) can never contend with them. Parallel answer runs on one target may race on session-bundle registration; session policy is resume-best-effort, so the cost is reduced continuity, not corruption.
