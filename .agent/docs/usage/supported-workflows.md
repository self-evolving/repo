...
For issue and pull request bodies, issue and pull request comments, and pull request review comments, the portal attempts the initial 👀 reaction as its first step, before checkout and runtime setup, using only `jq`, `gh`, raw event IDs, and REST with `github.token`. The inline check approximates the boundary-aware mention validation (fenced code, inline code, and quoted lines are ignored) and is best-effort; a successful early reaction suppresses the later resolved-identity reaction so two accounts do not both react. Edited events, discussions, and pull request review bodies keep the normal post-setup reaction path.
