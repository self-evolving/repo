...
4. Before onboarding, confirm the repository is ready:
   ...
   - At least one model-provider credential is configured: `OPENAI_API_KEY` for API-backed Codex, managed Codex account auth for trusted private CI, or `CLAUDE_CODE_OAUTH_TOKEN` / `ANTHROPIC_API_KEY` for Claude-backed runs.
