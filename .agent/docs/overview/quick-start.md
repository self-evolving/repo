...
4. Before onboarding, confirm the repository is ready:
   ...
   - At least one model-provider credential is configured under `Settings > Secrets and variables > Actions` as a repository secret: `OPENAI_API_KEY` for Codex-backed runs, `CLAUDE_CODE_OAUTH_TOKEN` / `ANTHROPIC_API_KEY` for Claude-backed runs, or `PI_AUTH_JSON_B64` / `PI_AUTH_JSON` plus `AGENT_DEFAULT_PROVIDER=pi` for Pi-backed runs.
