...
//      RUBRICS_PROPOSAL_REF, RUBRICS_PROPOSAL_HAS_DIFF, RUBRICS_WRITE_MODE,
//      RUBRIC_PR_ACTION, RUBRIC_PR_OUTCOME, RUBRIC_PR_URL,
//      RUBRICS_STEP_OUTCOME, RUBRICS_VALIDATION_OUTCOME,
//      RUBRICS_COMMIT_OUTCOME, TARGET_KIND, TARGET_NUMBER, GITHUB_REPOSITORY
// Outputs: body_file
...
const proposalHasDiff = process.env.RUBRICS_PROPOSAL_HAS_DIFF === "true";
...
const persistenceSucceeded =
  validationOutcome === "failure" || commitOutcome === "failure"
    ? false
    : undefined;
const effectiveRubricsCommitted =
  rubricsCommitted ||
  (writeMode === "proposal_pr" && proposalHasDiff);
const proposalPrSucceeded =
  writeMode === "proposal_pr" &&
  (rubricsCommitted || proposalHasDiff) &&
  proposalPrOutcome === "failure"
    ? false
    : undefined;
...
const body = formatAddRubricsComment({
  ...
  rubricsTargetRef,
  writeMode,
  rubricsCommitted: effectiveRubricsCommitted,
  runSucceeded,
