...
//      AGENT_PROGRESS_FINAL_COMMENT_MODE, AGENT_PROGRESS_GITHUB_TOKEN, MODEL_DISPLAY
...
function requestedByHumanLine(): string {
  if (normalizeToken(sourceTargetKind) !== "pull_request") return "";
  const raw = requestedBy.trim();
  if (!raw || /^app\//i.test(raw) || /\[bot\]$/i.test(raw)) return "";

  const login = raw.replace(/^@+/, "");
  if (!/^[a-z\d](?:[a-z\d-]{0,37}[a-z\d])?$/i.test(login)) return "";
...
function startOrchestrationComment(heading: string): string[] {
  const lines = [heading];
...
function isSuccessfulPrSourceOutcome(): boolean {
  ...
  if (action === "review") {
    return (
      conclusion === "ship" &&
      !allowSelfApprove &&
      (!recommendedNextStep || recommendedNextStep === "no_automated_action")
    );
  }
  if (action === "agent_self_approve") {
    return conclusion === "approved" && !allowSelfMerge;
  }
  return action === "agent_self_merge" && ["merged", "auto_merge_enabled"].includes(conclusion);
}
...
function hasSubstantiveTerminalSummary(decision: HandoffDecision): boolean {
  return Boolean(String(decision.userMessage || "").trim());
}
...
function formatPlannerClarificationComment(decision: HandoffDecision): string | null {
  ...
  const lines = startOrchestrationComment(
...
function formatPlannerAnswerComment(decision: HandoffDecision): string | null {
  ...
  const lines = startOrchestrationComment("Sepo answered this orchestration request.");
  lines.push(
    "",
    message,
    "",
...
function formatOrchestrateStopComment(decision: HandoffDecision): string {
  ...
  const successful = isValidatedSuccessfulPrStop(decision);
  const lines = startOrchestrationComment(
    successful
...
function currentFinalCommentId(prNumber: number, knownCommentId: string): string {
  if (/^\d+$/.test(knownCommentId.trim())) return knownCommentId.trim();
  const comments = fetchIssueComments(repo, prNumber).filter((comment) => (
    comment.id &&
    String(comment.body || "").includes(ORCHESTRATE_STOP_MARKER) &&
    isTrustedActorLogin(comment.authorLogin || "")
  ));
  return String(comments[comments.length - 1]?.id || "");
}

function collapsePrArtifactsForIdentity(
  prNumber: number,
  finalCommentId: string,
  identityLabel: string,
  client?: GraphQLClient,
): number {
  try {
    return collapsePreviousPrConversationArtifacts({
      repo,
      prNumber,
      finalBodyMarker: ORCHESTRATE_STOP_MARKER,
      currentCommentDatabaseId: finalCommentId,
      ...(client ? { client } : {}),
    });
  } catch (err: unknown) {
    console.warn(
      `Failed to collapse ${identityLabel} previous orchestration artifacts for ${repo}#${prNumber}: ${errorText(err)}`,
    );
    return 0;
  }
}
...
function collapseSuccessfulPrArtifacts(
  ...
  if (
    !collapseOldReviews ||
    !isValidatedSuccessfulPrStop(decision) ||
    !hasSubstantiveTerminalSummary(decision)
  ) {
    return;
  }
  ...
  let finalCommentId = "";
  try {
    finalCommentId = currentFinalCommentId(prNumber, knownCommentId);
  } catch (err: unknown) {
    console.warn(
      `Failed to resolve the terminal orchestration note for cleanup on ${repo}#${prNumber}: ${errorText(err)}`,
    );
    return;
  }
  if (!finalCommentId) return;

  let collapsed = collapsePrArtifactsForIdentity(
    prNumber,
    finalCommentId,
    "agent-authored",
  );
  ...
  if (progressGithubToken && progressGithubToken !== ambientGithubToken) {
    collapsed += collapsePrArtifactsForIdentity(
      prNumber,
      finalCommentId,
      "workflow-authored",
      createGhGraphqlClient({ token: progressGithubToken }),
    );
  }
  if (collapsed > 0) {
...
let orchestrateStopCommentHandled = false;
...
function createOrchestrateStopComment(decision: HandoffDecision): void {
  if (orchestrateStopCommentHandled) return;
  const target = parsePositiveTargetNumber(targetNumber);
  ...
  if (targetKind === "pull_request") {
    ...
    collapseSuccessfulPrArtifacts(target, decision, merged ? progressCommentId : "");
    return;
  }
...
if (decision.decision !== "dispatch" && decision.decision !== "delegate_issue") {
  ...
  try {
    ...
    console.error(`Failed to publish terminal pull request note: ${errorText(err)}`);
    process.exit(1);
  }
  process.exit(0);
}
