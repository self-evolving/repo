...
test("validateSepoVersionMetadata enforces version channel policy", () => {
  ...
  assert.throws(
    () => validateSepoVersionMetadata(validMetadata({ version: "1.0.0-rc.0", channel: "release-candidate" })),
...
test("validateSepoVersionMetadata rejects partial source identity for release installs", () => {
  assert.throws(
  ...
  assert.throws(
    ...
    /source_sha must be null or a full Git commit SHA/,
  );
  assert.throws(
...
test("formatSepoVersionSummary prints source identity fields", () => {
  assert.equal(
    formatSepoVersionSummary(validMetadata({ source_sha: fullSha, agent_files_hash: agentFilesHash })),
