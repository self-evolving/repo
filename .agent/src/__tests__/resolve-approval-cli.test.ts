...
test("resolve-approval applies downstream implement policy to release approvals", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "agent-resolve-approval-"));
  ...
  try {
    ...
    const fakeGh = join(tempDir, "gh");
    const marker = buildApprovalRequestMarker({
      request_id: "req-r3l345",
      route: "release",
      target_kind: "issue",
      target_number: 140,
      target_url: "https://github.com/self-evolving/repo/issues/140",
      workflow: "agent-implement.yml",
      issue_title: "Prepare Sepo release 0.2.0",
      issue_body: "Prepare release 0.2.0",
      request_text: "@sepo-agent /release 0.2.0",
    });
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        comment: {
          id: 104,
          node_id: "IC_104",
          body: "@sepo-agent /approve req-r3l345",
          author_association: "CONTRIBUTOR",
          user: { login: "alice" },
        },
        issue: {
          number: 140,
          html_url: "https://github.com/self-evolving/repo/issues/140",
        },
      }),
      "utf8",
    );
    ...
    writeFileSync(
      ...
      `#!/usr/bin/env bash
if [ "$1" = "api" ]; then
  printf '[{"id":203,"created_at":"2026-04-23T00:00:00Z","body":%s}]\\n' "$(node -e 'process.stdout.write(JSON.stringify(process.env.MARKER_BODY))')"
  exit 0
fi
exit 1
`,
