...
test("extract-context prefers a newly added edited fix-pr after an earlier route", () => {
  const outputs = runExtractContextCli({
    eventName: "issue_comment",
    payload: {
      action: "edited",
  ...
  const outputs = runExtractContextCli({
    ...
    payload: {
      ...
      comment: {
        id: 403,
        node_id: "IC_403",
        html_url: "https://github.com/self-evolving/repo/pull/403#issuecomment-403",
      ...
      issue: {
        ...
        pull_request: { url: "https://api.github.com/repos/self-evolving/repo/pulls/403" },
      },
    },
  });
