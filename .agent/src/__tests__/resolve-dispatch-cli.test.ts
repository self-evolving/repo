...
test("resolve-dispatch emits add-rubrics write mode from request wording", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "agent-resolve-dispatch-"));
  ...
  try {
    ...
    const result = spawnSync("node", [".agent/dist/cli/resolve-dispatch.js"], {
      ...
      env: {
        ...
        REQUEST_TEXT: "@sepo-agent please add this rubric and commit directly to agent/rubrics",
        TARGET_KIND: "issue",
        AUTHOR_ASSOCIATION: "MEMBER",
        ACCESS_POLICY: "",
        REPOSITORY_PRIVATE: "true",
    ...
    assert.equal(outputs.get("rubrics_write_mode"), "direct_commit");
