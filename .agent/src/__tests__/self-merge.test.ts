...
test("formatSelfMergeBody includes visible status and marker", () => {
  const body = formatSelfMergeBody({
    conclusion: "waiting",
    reason: "pull request base agent/parent is not the default branch main; waiting on the base PR before self-merge",
    runUrl: "https://github.com/self-evolving/repo/actions/runs/123",
  });

  assert.match(body, /\| Waiting \| `waiting` \|/);
  assert.match(body, /waiting on the base PR/);
