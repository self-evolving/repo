...
test("install prompt keeps fork mechanics in helpers and install judgment in the agent", () => {
  const installPrompt = readRepoFile(".github/prompts/agent-install.md");

  assert.match(installPrompt, /resolve-install-target\.js/);
  assert.match(installPrompt, /install-fork-pr\.js prepare/);
  assert.match(installPrompt, /install-fork-pr\.js publish/);
  assert.match(installPrompt, /The fork\/PR helper owns target GitHub mechanics/);
  assert.match(installPrompt, /target-specific inspection, copy, validation, commit, and PR body judgment/);
  assert.match(installPrompt, /Merge `\.agent\/dist\/` and `\.agent\/node_modules\/` into the target/);
  assert.doesNotMatch(installPrompt, new RegExp(["plan", "install", "lifecycle"].join("-")));
  assert.doesNotMatch(installPrompt, /lifecycle\s+plan/);
  assert.doesNotMatch(installPrompt, new RegExp(`ordered ${"`"}steps${"`"}`));
});
