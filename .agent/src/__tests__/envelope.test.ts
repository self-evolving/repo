...
test("review synthesis uses a shared reviews directory contract", () => {
  ...
  assert.match(reviewWorkflow, /AGENT_INLINE_COMMENT_CLEANUP_MODE:\s*\$\{\{\s*vars\.AGENT_INLINE_COMMENT_CLEANUP_MODE\s*\|\|\s*'resolve'\s*\}\}/);
  assert.match(reviewPrompt, /gh api --paginate repos\/\$\{REPO_SLUG\}\/pulls\/\$\{TARGET_NUMBER\}\/comments/);
  assert.match(reviewPrompt, /inline cleanup mode is `\$\{AGENT_INLINE_COMMENT_CLEANUP_MODE\}`/);
  ...
  assert.match(reviewPrompt, /Only suggest `resolve_existing_thread` when inline cleanup mode is `resolve`/);
  assert.match(reviewPrompt, /Only suggest `mark_existing_outdated` when inline cleanup mode is `minimize`/);
  ...
  assert.match(synthesisPrompt, /Inline cleanup mode is `\$\{AGENT_INLINE_COMMENT_CLEANUP_MODE\}`/);
  ...
  assert.match(synthesisPrompt, /re-fetch existing inline comments and review threads when relevant[\s\S]*verify\s+the target still belongs to this PR/);
  ...
  assert.match(synthesisPrompt, /every thread comment authored by the same authenticated agent account/);
  assert.match(synthesisPrompt, /never resolve human threads or threads from other bots/);
  assert.match(synthesisPrompt, /minimizeComment\(input: \{ subjectId: \$id, classifier: OUTDATED \}\)/);
  assert.match(synthesisPrompt, /Only minimize comments authored by the same authenticated\s+agent account/);
  assert.match(synthesisPrompt, /never minimize\s+human comments or comments from other bots/);
  assert.match(synthesisPrompt, /when inline cleanup mode is `off`, do not resolve or minimize older inline\s+comments/);
  assert.match(synthesisPrompt, /do not delete inline comments/);
  assert.match(synthesisPrompt, /do not resolve when inline cleanup mode is not `resolve`/);
  assert.match(synthesisPrompt, /do not minimize when inline cleanup mode is not `minimize`/);
  assert.match(synthesisPrompt, /do not reply to, resolve, or minimize anything when authorship, PR ownership/);
  assert.match(synthesisPrompt, /Progress` section/);
  assert.match(runSource, /"REVIEWS_DIR"/);
  assert.match(runSource, /"AGENT_INLINE_COMMENT_CLEANUP_MODE"/);
