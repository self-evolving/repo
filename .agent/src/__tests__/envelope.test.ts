...
test("review synthesis uses a shared reviews directory contract", () => {
  ...
  assert.match(reviewPrompt, /open_new[\s\S]*reply_existing[\s\S]*resolve_existing_thread[\s\S]*no_action/);
  ...
  assert.match(synthesisPrompt, /re-fetch existing inline comments and review threads[\s\S]*verify the target\s+still belongs to this PR/);
  ...
  assert.match(synthesisPrompt, /do not reply to or resolve anything when authorship, PR ownership/);
  assert.match(synthesisPrompt, /do not delete or minimize inline comments/);
