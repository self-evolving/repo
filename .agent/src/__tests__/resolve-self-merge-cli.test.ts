...
printf '%s\\n' "$*" >> "$FAKE_GH_LOG"
if [ "$1" = "pr" ] && [ "$2" = "view" ]; then
  printf '{"baseRefName":"%s","headRefOid":"abc123","isDraft":%s,"state":"%s","mergeStateStatus":"%s","mergeable":"%s","reviewDecision":"%s","statusCheckRollup":%s,"autoMergeRequest":null}\\n' \
    "\${FAKE_BASE_REF-main}" \
    "\${FAKE_IS_DRAFT-false}" \
    "\${FAKE_PR_STATE-OPEN}" \
...
test("resolve-self-merge posts waiting status for non-default bases", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "agent-self-merge-cli-"));
  try {
    writeFakeGh(tempDir);
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.equal(result.outputs.get("conclusion"), "waiting");
    assert.equal(result.outputs.get("status_post"), "true");
    assert.match(result.outputs.get("reason") || "", /waiting on the base PR/);
