...
export const DEFAULT_SEPO_VERSION_METADATA_PATH = join(
  __dirname,
  "..",
  "sepo-version.json",
);
...
export function validateSepoVersionMetadata(value: unknown): SepoVersionMetadata {
  ...
  if (sourceSha && !FULL_GIT_SHA_RE.test(sourceSha)) {
    throw new Error("source_sha must be null or a full lowercase Git commit SHA");
  }
...
export function readSepoVersionMetadata(
  ...
  return parseSepoVersionMetadataJson(readFileSync(metadataPath, "utf8"));
}
