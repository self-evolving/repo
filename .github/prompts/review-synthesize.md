...
Inline cleanup mode is `${AGENT_INLINE_COMMENT_CLEANUP_MODE}`. Valid values are
`resolve`, `minimize`, and `off`; treat an empty value as `resolve`. If the
value is unsupported, do not resolve or minimize older inline comments and
mention the invalid mode in the final synthesis `Progress` section.
Use GraphQL `reviewThreads` to inspect existing inline review threads before
resolving any thread, for example:
...
re-fetch existing inline comments and review threads when relevant, and verify
the target still belongs to this PR and still warrants the action.
...
- when inline cleanup mode is `resolve`, for `resolve_existing_thread`, you may
  resolve older same-agent inline review threads when the current synthesis
  confirms the thread's issue has been addressed or superseded. First re-fetch
  the PR's `reviewThreads` and check the target thread `id`, `isResolved`,
  `viewerCanResolve`, `path`, `line`, and comments' authorship. Only resolve
  unresolved threads that belong to this PR, are resolvable by the viewer, and
  have every thread comment authored by the same authenticated agent account;
  never resolve human threads or threads from other bots. Use:
...
- when inline cleanup mode is `minimize`, for `mark_existing_outdated`, you may
  mark older same-agent inline comments as outdated when the current synthesis
  supersedes them. Only minimize comments authored by the same authenticated
  agent account, only use the existing comment's `node_id`, and never minimize
  human comments or comments from other bots. Use:
...
- when inline cleanup mode is `off`, do not resolve or minimize older inline
  comments
- do not delete inline comments
- do not resolve when inline cleanup mode is not `resolve`
- do not minimize when inline cleanup mode is not `minimize`
- do not reply to, resolve, or minimize anything when authorship, PR ownership,
  `viewerCanResolve` for resolution, whether the issue was addressed, or
  whether the new synthesis supersedes the old inline comment is uncertain
