...
Prepare a Sepo release pull request for the version requested in GitHub issue #${TARGET_NUMBER}.
...
2. Identify the requested version from the issue title/body or latest human request. If it is missing or ambiguous, make no file changes and explain the blocker in the JSON `summary`.
3. Validate the requested version against `.agent/docs/technical-details/versioning.md`.
...
7. Update release documentation or checklist entries that should change for this version.
...
- `pr_title` should be specific to the requested release version.
