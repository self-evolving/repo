...
6. Update `.agent/CHANGELOG.md` with release notes for this version, drafted from the pull requests merged since the previous release tag. Pull requests do not maintain an `Unreleased` section, so derive the content from the merged history rather than expecting pre-accumulated entries.
