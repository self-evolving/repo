...
- If the latest review synthesis includes a `Recommended Next Step`, treat it
  ...
  it or self-approval is enabled for a review `HUMAN_DECISION`.
- When source recommended next step is `HUMAN_DECISION` and self-approval is
  enabled, hand off to `agent-self-approve` with the source handoff context,
  regardless of whether the review verdict is `SHIP`, `MINOR_ISSUES`, or
  `NEEDS_REWORK`. Let self-approval approve only when safe; otherwise it should
  request changes or return `BLOCKED` if the decision should remain with a human.
