...
- For `stop`, include a concise `user_message` summarizing the cumulative work
  and terminal outcome from the current target's orchestration history. If no
  substantive work completed, explain the remaining human action or blocker.
