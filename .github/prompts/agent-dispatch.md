...
- `orchestrate`: start the orchestrator workflow immediately; only valid for `issue`, `pull_request`, or `discussion`
...
- Use `orchestrate` when the user explicitly asks for orchestration, follow-up automation, or a bounded multi-step agent workflow on an issue, pull request, or discussion.
...
- `orchestrate` is only valid for `issue`, `pull_request`, and `discussion` targets. If the request is on another target kind, use `unsupported`.
