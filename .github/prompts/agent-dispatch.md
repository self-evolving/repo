...
- Use `setup` only for plan-only setup requests, especially `@sepo-agent /setup plan` on a Sepo setup issue. Do not use it for apply/create/link requests.
...
- `setup` is only valid for `issue` targets. If the setup request asks to apply changes, create/link Projects, or mutate repository variables, use `unsupported` because `/setup apply` is not implemented yet.
